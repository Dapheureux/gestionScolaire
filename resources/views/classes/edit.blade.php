...
<body class="bg-gray-100 font-sans flex">
    ...
    <div class="w-4/5 p-10">
        ...
        <form action="{{ route('classes.update', ['classe' => $classe->id]) }}" method="post" class="mb-6">
            @csrf
            @method('PATCH') <!-- Utiliser la méthode PATCH pour la mise à jour -->

            <div class="mb-4">
                <label for="nom" class="block text-gray-700 text-sm font-bold mb-2">Nom de la classe</label>
                <input type="text" name="nom" id="nom" value="{{ old('nom', $classe->nom) }}" class="w-full px-4 py-2 border rounded-md" required>
                @error('nom')
