    ...
    <aside class="bg-blue-800 text-white w-1/5 p-4 shadow-md min-h-screen">
        ...
        <ul>
            ...
            <li class="mb-2 flex p-3 border rounded-lg">
                ...
                <a href="#" class="hover:text-gray-300">Élèves</a>
            </li>
            <li class="mb-2 flex p-3 border rounded-lg">
                <svg class="mr-2 w-6 h-6" fill="none" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M4.26 10.147a60.438 60.438 0 0 0-.491 6.347A48.62 48.62 0 0 1 12 20.904a48.62 48.62 0 0 1 8.232-4.41 60.46 60.46 0 0 0-.491-6.347m-15.482 0a50.636 50.636 0 0 0-2.658-.813A59.906 59.906 0 0 1 12 3.493a59.903 59.903 0 0 1 10.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.717 50.717 0 0 1 12 13.489a50.702 50.702 0 0 1 7.74-3.342M6.75 15a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5Zm0 0v-3.675A55.378 55.378 0 0 1 12 8.443m-7.007 11.55A5.981 5.981 0 0 0 6.75 15.75v-1.5" />
                </svg>
                <a href="{{ route('Classes') }}" class="hover:text-gray-300">Classes</a>
            </li>
            <li class="mb-2 flex p-3 border rounded-lg">
                <svg class="mr-2 w-6 h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
    ...
    <div class="w-4/5 p-10">
        ...
        <div>
           ...
           <h2 class="text-2xl font-bold mb-4">Liste des Élèves</h2>
            <div>
            <a href="{{ route('Eleves.create') }}" class="px-2 py-1 bg-blue-500 text-white rounded-md hover:bg-blue-600">Ajouter un élève</a>
            <button class="px-2 py-1 bg-blue-500 text-white rounded-md hover:bg-blue-600">Imprimer la liste</button>
            </div>
           </div>
            <ul>
                @foreach($eleves as $eleve)
                    <li id="eleve-{{ $eleve->id }}" class="flex items-center justify-between px-4 py-2 bg-white shadow rounded-md mb-2">
                        <span>{{ $eleve->nom }}</span>
                        <div>
                            <a href="{{route('eleves.edit', ['eleve' => $eleve->id])}}" class="px-2 py-1 bg-yellow-500 text-white rounded-md hover:bg-yellow-600 mr-2">Modifier</a>
                            <button onclick="deleteEleve({{ $eleve->id }})" class="px-2 py-1 bg-red-500 text-white rounded-md hover:bg-red-600">Supprimer</button>
                        </div>
                    </li>
                @endforeach
            </ul>
    ...
    <script>
        function deleteEleve(id) {
            if (window.confirm('Voulez-vous vraiment supprimer cet élève ?')) {
                ...
                .then(data => {
                    if (data.success) {
                        const eleveElement = document.getElementById(`eleve-${id}`);
                        if (eleveElement) {
                            eleveElement.parentNode.removeChild(eleveElement);
                            alert('Élève supprimé avec succès.');
                        }
                    } else {
                        alert('Une erreur s\'est produite lors de la suppression de l\'élève.');
                    }
                })
                .catch(error => {
